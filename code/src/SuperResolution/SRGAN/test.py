...
def sr_forward(img, padding=0.5, moving=0.1):
    ...
    input_img = torch.unsqueeze(torch.FloatTensor(img), 0)
    ...
    # output_img = np.clip(np.transpose(output_img, (1, 2, 0)), 0, 255).astype(np.uint8)
    # output_img = np.clip((np.transpose(output_img, (1, 2, 0)) / 2.0 + 0.5), 0, 255).astype(np.uint8)
    output_img = np.clip((np.transpose(output_img, (1, 2, 0)) / 10.0 + 0.5) * 2.0, 0, 255).astype(np.uint8)
...
# TODO: Hier Import versuchen von A2
face_paths, labels = DataLoader().load_A2()

img_path = 'input.jpg'
images, file_names = utils.read_cv2_img(face_paths)

id = 0
for image, file_name in zip(images, file_names):
    # output_img, rec_img = sr_forward(image)
    output_img = sr_forward(image)
    utils.save_image(output_img, 'test_sr/{}'.format(file_name))
    # utils.save_image(rec_img, 'output_img_{}.jpg'.format(id))
    id += 1
